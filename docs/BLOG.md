...
the same architecture twice and you get byte-identical findings and the same cost total,
because the parts a reviewer would take at face value never pass through a model at all.
...
| **AgentCore Memory** | Short-term summarization plus long-term semantic and user-preference recall across reviews. |
| **AgentCore Gateway** | Exposes an AWS Documentation MCP server as a tool, so an "AWS recommends X" claim is grounded in a live documentation fetch rather than model memory. |
| **AgentCore Identity** | Mints the Gateway's OAuth token via an M2M flow — no client secret sits in an environment variable. |
| **AgentCore Policy Engine** | Cedar policies in `ENFORCE` mode make read-only tool access a platform constraint, not a prompt instruction. |
| **AgentCore Code Interpreter** | Executes model-authored cost arithmetic in a managed sandbox and returns the executed code alongside the answer, so the result is auditable rather than asserted. |
| **Bedrock Knowledge Bases** | Vector search over a curated FAQ. Retrieval only — no generation call. |
...
| **Amazon CloudWatch** | Logs and metrics for the Lambda and the AgentCore Runtime, including OpenTelemetry-based traces from AgentCore's observability integration. |
